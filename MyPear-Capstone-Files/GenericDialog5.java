/**
* Modified version of ImageJ GenericDialog to work with MyPear plugin.
*/
...
public class GenericDialog5 extends Dialog implements ActionListener, TextListener,
FocusListener, ItemListener, KeyListener, AdjustmentListener, WindowListener {
	...
	public GenericDialog5(String title, int x, int y) {
    ...
    public GenericDialog5(String title) {
    	this(title, WindowManager.getCurrentImage()!=null?
			(Frame)WindowManager.getCurrentImage().getWindow():IJ.getInstance()!=null?IJ.getInstance():new Frame());
	}

    /** Creates a new GenericDialog using the specified title and parent frame. */
    public GenericDialog5(String title, Frame parent) {
   ...
   */
	...
	*/
	public void addSlider(String label, double minValue, double maxValue, double defaultValue) {
		...
		ToolTipManager.setToolTipText(s, "Drag the slider to select a value.");
		GUI.fix(s);
		slider.addElement(s);
		...
		ToolTipManager.setToolTipText(tf, "Click here to enter a value.");
		if (IJ.isLinux()) tf.setBackground(Color.white);
		tf.addActionListener(this);
		tf.addTextListener(this);
		tf.addFocusListener(this);
   ...
   public String getNextString() {
	...
	public void showDialog() {
		if (macro) {
			...
			ToolTipManager.setToolTipText(cancel, "Click Cancel to close the dialog.");
			if (yesNoCancel) {
				okLabel = yesLabel;
			...
			ToolTipManager.setToolTipText(okay, "Click Create to calculate and draw contour lines.");
		...
		if (!wasCanceled && dialogListeners!=null && dialogListeners.size()>0) {
			resetCounters();
			((DialogListener5)dialogListeners.elementAt(0)).dialogItemChanged(this,null);
			recorderOn = false;
		}
		resetCounters();
	}
...
/** Returns the Vector containing the numeric TextFields. */
    ...
    private void notifyListeners(AWTEvent e) {
        ...
        for (int i=0; everythingOk && i<dialogListeners.size(); i++)
            try {
                resetCounters();
                if (!((DialogListener5)dialogListeners.elementAt(i)).dialogItemChanged(this, e))
