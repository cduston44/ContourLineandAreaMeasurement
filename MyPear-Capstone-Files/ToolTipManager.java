/**
* Modified version of ToolTipManager to work with MyPear plugin.
*/
...
public class ToolTipManager implements MouseListener, Runnable {
	...
	/**
	 * The number of milliseconds that the mouse must remain over a Component before
	 * its tooltip is displayed.
	 */
	private static final int WAIT_TIME = 1000; // milliseconds

	/**
	 * Only one ToolTipManager object per application is needed so a singleton is
	 * created.
	 */
	...
	/**
	 * A thread used to determine whether the cursor has remained over a Component
	 * long enough to display its tooltip.
	 */
	...
	/**
	 * Creates a ToolTipManager. This is only invoked from this class since only a
	 * singleton is needed.
	 */
	...
	/**
	 * Prevents the tooltip from being displayed if the timer hasn't expired and
	 * hides the tooltip window if it has.
	 */
	...
	/**
	 * Determines which Component the mouse cursor is over and starts a thread to
	 * wait the appropriate amount of time before displaying its tooltip.
	 */
	...
	/**
	 * Interrupts the thread that is waiting to display the tooltip and hides the
	 * tooltip window.
	 */
	...
	/**
	 * Implementation of the timer thread. This waits the appropriate amount of time
	 * and then displays the tooltip.
	 */
	public synchronized void run() {
		while (true) {
			try {
				...
				String tip = (String) componentToTipTable.get(currentComponent);
				label.setText(tip);
				...
				java.awt.Point location = currentComponent.getLocationOnScreen();
